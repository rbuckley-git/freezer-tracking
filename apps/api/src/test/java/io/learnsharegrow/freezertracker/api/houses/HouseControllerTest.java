...
class HouseControllerTest {
  ...
  @Test
  void shouldAllowSuperAdminToManageHouses() throws Exception {
    SeededAccount admin = seedAccount("admin.house@example.com", "admin-pass", true);

    HouseCreateRequest createRequest = new HouseCreateRequest();
    createRequest.setName("New house " + UUID.randomUUID());
    ...
    HouseResponse created = objectMapper.readValue(createResponse, HouseResponse.class);

    HouseUpdateRequest updateRequest = new HouseUpdateRequest();
    updateRequest.setName("Updated house " + UUID.randomUUID());

    mockMvc
        .perform(
            put("/houses/{id}", created.getId())
                .header("X-API-Key", admin.apiKey().toString())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(delete("/houses/{id}", created.getId()).header("X-API-Key", admin.apiKey().toString()))
        .andExpect(status().isNoContent());
  }

  @Test
  void shouldRejectNonSuperAdminHouseAccess() throws Exception {
    SeededAccount user = seedAccount("user.house@example.com", "user-pass", true, false);

    mockMvc.perform(get("/houses").header("X-API-Key", user.apiKey().toString())).andExpect(status().isForbidden());
  }
  ...
  private SeededAccount seedAccount(String username, String password, boolean isAdmin, boolean isSuperAdmin) {
    ...
    house.setName("Account house " + UUID.randomUUID());
